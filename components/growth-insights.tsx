...
export function GrowthInsights() {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="border rounded-lg p-4">
                    {(() => {
                      const currentDemand =
                        demandPredictionData.filter((d) => !d.predicted).length > 0
                          ? demandPredictionData.filter((d) => !d.predicted)[
                              demandPredictionData.filter((d) => !d.predicted).length - 1
                            ].quantity
                          : 0
                      ...
                      const growthPercent =
                        currentDemand > 0 ? ((futureDemand - currentDemand) / currentDemand) * 100 : 0
                      ...
                      return (
                        <div className="text-center">
                          ...
                          <div className="flex items-center justify-center">
                            ...
                            <div className="mx-4">
                              <p
                                className={`text-lg font-semibold ${growthPercent >= 0 ? "text-green-500" : "text-red-500"}`}
                              >
                                {growthPercent >= 0 ? "+" : ""}
                ...
                <AlertDescription>
                  {pricingOptimizationData.length > 0 ? (
                    <>
                      ...
                      {(
                        pricingOptimizationData.sort((a, b) => b.profitMargin - a.profitMargin)[0].profitMargin * 100
                      ).toFixed(1)}
                      %.
                    </>
                  ) : (
                    ...
                    .map((range, index) => (
                      <div key={index} className="flex justify-between items-center border-b pb-2">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{range.orderCount} orders</p>
                        </div>
            ...
            <div className="space-y-4 h-[300px] overflow-auto">
              {materialPurchaseData.length > 0 ? (
                materialPurchaseData.slice(0, 5).map((rec, index) => (
                  <div key={index} className="flex items-center justify-between border-b pb-2">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{format(new Date(rec.date), "dd MMM yyyy")}</p>
                    </div>
                    <div className="text-right">
                ...
                <Alert>
                  ...
                  <AlertDescription>Insufficient price data to make purchase recommendations.</AlertDescription>
                </Alert>
              )}
            </div>
              ...
              {materialPurchaseData.length > 0 && (
                <div className="border rounded-lg p-4">
                  ...
                  <div className="mt-4 pt-4 border-t">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Monitor price trends and purchase materials when they reach local price minima to maximize
                      savings.
                    </p>
                  </div>
                </div>
      ...
      <Card className="col-span-full">
        <CardHeader>
          <CardTitle>Profit Maximization Analytics</CardTitle>
          <CardDescription>Identify the most profitable products and opportunities</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={profitMaximizationData.slice(0, 10)}>
                  <CartesianGrid strokeDasharray="3 3" />
                  ...
                  <YAxis
                    ...
                    orientation="left"
                    tickFormatter={(value) => `₹${(value / 1000).toFixed(0)}K`}
                    label={{ value: "Total Profit (₹)", angle: -90, position: "insideLeft" }}
                  />
                  <YAxis
                    yAxisId="right"
                    orientation="right"
                  ...
                  <Tooltip
                    formatter={(value, name) => {
                      if (name === "profitMargin") return [`${(Number(value) * 100).toFixed(1)}%`, "Profit Margin"]
                      if (name === "totalProfit") return [`₹${Number(value).toLocaleString()}`, "Total Profit"]
                      return [value, name]
                    }}
                  />
                  <Legend />
                  <Bar yAxisId="left" dataKey="totalProfit" fill="hsl(var(--chart-1))" name="Total Profit" />
                  <Bar yAxisId="right" dataKey="profitMargin" fill="hsl(var(--chart-2))" name="Profit Margin" />
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="space-y-4">
              <h3 className="text-lg font-semibold">Profit Insights</h3>

              <Alert>
                <TrendingUp className="h-4 w-4" />
                <AlertTitle>Most Profitable Products</AlertTitle>
                <AlertDescription>
                  {profitMaximizationData.length > 0 ? (
                    <>
                      {profitMaximizationData[0].product} generates the highest total profit at ₹
                      {profitMaximizationData[0].totalProfit.toLocaleString()}.
                    </>
                  ) : (
                    "Insufficient data for profit analysis."
                  )}
                </AlertDescription>
              </Alert>
              ...
              {profitMaximizationData.length > 0 && (
                <div className="space-y-3">
                  <p className="text-sm">Product profitability:</p>

                  {profitMaximizationData.slice(0, 3).map((product, index) => (
                    <div key={index} className="flex justify-between items-center border-b pb-2">
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {(product.profitMargin * 100).toFixed(1)}% margin
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <Alert>
                <TrendingUp className="h-4 w-4" />
                <AlertTitle>Profit Optimization</AlertTitle>
                <AlertDescription>
                  {profitMaximizationData.length > 0
                    ? `Focus marketing efforts on ${profitMaximizationData[0].product} and ${
                        profitMaximizationData.length > 1 ? profitMaximizationData[1].product : ""
                      } to maximize overall profitability.`
                    : "Upload more order data to receive profit optimization recommendations."}
                </AlertDescription>
              </Alert>
            </div>
          </div>
